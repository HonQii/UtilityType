        ...
        case .structDecl:
            ...
            let structRawProperties = targetStructProperties
                .map { structProperty in
                    ...
                    if let access {
                        return "\(access.description)\(letOrVar.drop(while: { $0.isWhitespace })) \(structProperty)"
                    } else {
                        return "\(letOrVar.drop(while: { $0.isWhitespace })) \(structProperty)"
                    }
                }
                .joined()
        ...
        case .classDecl:
            ...
            let classRawProperties = targetClassProperties
                .map { classProperty in
                    ...
                    let letOrVar = variableDecl.bindingSpecifier.text
                    if let access {
                        return "\(access.description)\(letOrVar.drop(while: { $0.isWhitespace })) \(classProperty)"
                    } else {
                        return "\(letOrVar.drop(while: { $0.isWhitespace })) \(classProperty)"
                    }
                }
                .joined()
